...
SA_API struct sb_ModelMesh* sb_model_mesh_load(const char* path) {
    struct sb_ModelMesh* mesh = su_MALLOC_M(sizeof(struct sb_ModelMesh));
...
void sb_model_delete(struct sb_ModelMesh* model_mesh) {
    su_FREE_M(model_mesh->index_array);
...
SA_API void sb_model_get_vertex_index_array(const struct sb_ModelMesh* model_mesh,
...
SA_API void sb_model_get_separated_index_data(const struct sb_ModelMesh* model_mesh,
    ...
    *index_array_count_out = model_mesh->index_count;
    ...
    if (vertex_index_out) {
        *vertex_index_out = (su_U32*)su_MALLOC_M(sizeof(su_U32) * (*index_array_count_out));
    }
    if (uv_index_out) {
        *uv_index_out = (su_U32*)su_MALLOC_M(sizeof(su_U32) * (*index_array_count_out));
    }
    if (normal_index_out) {
        *normal_index_out = (su_U32*)su_MALLOC_M(sizeof(su_U32) * (*index_array_count_out));
    }
...
SA_API void sb_model_get_position_array(const struct sb_ModelMesh* model_mesh,
    ...
    *position_array_out = su_MALLOC_M(sizeof(su_Vec3) * model_mesh->positions_count);
...
SA_API void sb_model_get_uv_array(const struct sb_ModelMesh* model_mesh,
                                  su_Uv** uv_array_out,
                                  su_U64* uv_count_out) {
    *uv_array_out = su_MALLOC_M(sizeof(su_Uv) * model_mesh->uv_count);
    *uv_count_out = model_mesh->uv_count;
    memcpy(*uv_array_out, model_mesh->uv_array,
           (sizeof(su_Uv) * model_mesh->uv_count));
}

SA_API void sb_model_vertex_index_get_data(const struct sb_VertexIndex* vertex_index,
                                           su_U64 vertex_index_amount,
                                           su_U32** vertex_index_out,
                                           su_U32** uv_index_out,
                                           su_U32** normal_index_out) {
    *vertex_index_out = su_MALLOC_M(sizeof(su_U32) * vertex_index_amount);
    *uv_index_out = su_MALLOC_M(sizeof(su_U32) * vertex_index_amount);
    *normal_index_out = su_MALLOC_M(sizeof(su_U32) * vertex_index_amount);
...
SA_INTERNAL su_Bool sb__model_parse(const char* file_path,
    ...
    *position_array_out = su_SCAST_TO_M(su_Vec3*)(su_MALLOC_M(sizeof(su_Vec3) * (*positions_count_out)));
    ...
    for (su_U64 i = 0; i < (*positions_count_out); ++i) {
        ...
        (*position_array_out)[i] = su_SCAST_TO_M(su_Vec3){pos_x, pos_y, pos_z};
    }

    *uv_count_out = attribute.num_texcoords;
    *uv_array_out = su_MALLOC_M(sizeof(su_Uv) * (*uv_count_out));
    ...
    *index_array_out = su_MALLOC_M(sizeof(struct sb_VertexIndex) * (*index_count_out));
    for (su_U64 i = 0; i < (attribute.num_faces); ++i) {
        (*index_array_out)[i].vertex_index = su_SCAST_TO_M(su_U32)(attribute.faces[i].v_idx);
        (*index_array_out)[i].uv_index = su_SCAST_TO_M(su_U32) attribute.faces[i].vt_idx;
        (*index_array_out)[i].normal_index = su_SCAST_TO_M(su_U32) attribute.faces[i].vn_idx;
    }
...
SA_INTERNAL void sb__file_reader_function(void* ctx, const char* filename, int is_mtl,
    ...
    su_U64 file_size = su_SCAST_TO_M(su_U64)(ftell(file));
    rewind(file);
