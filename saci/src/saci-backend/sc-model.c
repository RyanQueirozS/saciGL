...
SA_INTERNAL su_bool s_Model_Parse(const char* file_path,
                                  su_vec3** position_array_out, su_u64* positions_count_out,
                                  su_uv** texcoord_array_out, su_u64* texcoord_count_out,
                                  struct sc_vertexIndice** indice_array_out, su_u64* indices_count_out);
...
SA_API struct sc_modelMesh* sc_Model_Mesh_Load(const char* path) {
    ...
    su_bool success = s_Model_Parse(path,
                                    &mesh->position_array,
...
SA_API void sc_Model_Get_Separated_Indice_Data(const struct sc_modelMesh* model_mesh,
                                               ...
                                               su_u64* indice_array_count_out) {
    *indice_array_count_out = model_mesh->indices_count;

    if (vertex_index_out) {
        *vertex_index_out = (su_u32*)su_Malloc_m(sizeof(su_u32) * (*indice_array_count_out));
    }
    if (uv_index_out) {
        *uv_index_out = (su_u32*)su_Malloc_m(sizeof(su_u32) * (*indice_array_count_out));
    }
    if (normal_index_out) {
        *normal_index_out = (su_u32*)su_Malloc_m(sizeof(su_u32) * (*indice_array_count_out));
    }

    for (su_u64 i = 0; i < *indice_array_count_out; ++i) {
        if (vertex_index_out) {
...
SA_API void sc_Model_Vertex_Indice_Get_Data(const struct sc_vertexIndice* vertex_indice,
                                            ...
                                            su_u32** uv_index_out,
                                            su_u32** normal_index_out) {
    *vertex_index_out = su_Malloc_m(sizeof(su_u32) * vertex_indice_amount);
    *uv_index_out = su_Malloc_m(sizeof(su_u32) * vertex_indice_amount);
    *normal_index_out = su_Malloc_m(sizeof(su_u32) * vertex_indice_amount);
    for (su_u64 i = 0; i < vertex_indice_amount; ++i) {
...
SA_INTERNAL su_bool s_Model_Parse(const char* file_path,
    ...
    *indice_array_out = su_Malloc_m(sizeof(struct sc_vertexIndice) * (*indices_count_out));
    for (su_u64 i = 0; i < (attribute.num_faces); ++i) {
        (*indice_array_out)[i].vertex_index = su_Scast_To_m(su_u32)(attribute.faces[i].v_idx);
        (*indice_array_out)[i].uv_index = su_Scast_To_m(su_u32) attribute.faces[i].vt_idx;
        (*indice_array_out)[i].normal_index = su_Scast_To_m(su_u32) attribute.faces[i].vn_idx;
    }
