...
su_Bool sb_load_windowing(void) {
    ...
    windowing_dependent_funcs.glfw.set_hint(GLFW_CONTEXT_VERSION_MAJOR, 3);
    windowing_dependent_funcs.glfw.set_hint(GLFW_CONTEXT_VERSION_MINOR, 3);
    windowing_dependent_funcs.glfw.set_hint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);
...
su_Bool sb_proc_load(void) {
    ...
    su_LOG_INFOF_M(su_LOG_TYPE_PROD, su_LOG_CONTEXT_LIB_GLFW,
                   "Using OpenGL version: %s", render_dependent_funcs.gl.get_string(GL_VERSION));
    return su_TRUE;
}
...
void sb_window_set_pos_handler(su_Window window, su_WindowPosHandler window_pos_handler) {
    windowing_dependent_funcs.glfw.set_pos_handler(window, (void*)window_pos_handler);
...
void sb_window_set_size_handler(su_Window window, su_WindowSizeHandler window_size_handler) {
    windowing_dependent_funcs.glfw.set_size_handler(window, (void*)window_size_handler);
...
void sb_window_clear_color(su_Color color) {
    render_dependent_funcs.gl.clear_color(color.r, color.g, color.b, color.a);
    render_dependent_funcs.gl.clear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}
...
void sb__initialize_debugger(void) {
    render_dependent_funcs.gl.enable(GL_DEBUG_OUTPUT);
    render_dependent_funcs.gl.enable(GL_DEBUG_OUTPUT_SYNCHRONOUS);
    render_dependent_funcs.gl.enable(GL_DEBUG_OUTPUT_SYNCHRONOUS);
    // render_dependent_funcs.gl.debug_message_callback(su_log_opengl_debug_message_callback, NULL);
...
SA_API su_Bool sb_event_is_key_pressed(su_Window window, int keycode) {
    return windowing_dependent_funcs.glfw.is_key_pressed(window, su_SCAST_TO_M(int)(keycode)) == GLFW_PRESS;
}
